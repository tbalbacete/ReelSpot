...
export const IndividualMovie: React.FC = () => {
  ...
  const {
    ...
    voteAverage,
    overview,
  }: MovieDetails = movieQuery.data ?? {};
  const crew: CrewMember[] = movieCreditsQuery?.data?.crew;
  const trailer: Video = movieVideosQuery.data?.results.find(
    (video: Video) => video.type === "Trailer"
  );

  useEffect(() => {
    document.title = `${title} (${dayjs(releaseDate).format(
      "YYYY"
    )}) - ReelSpot`;
  });
  ...
  const rating =
    movieReleasesQuery.data?.results
      .find((country: Country) => country.iso_3166_1 === "US")
      ?.release_dates?.find((release: Release) => release.certification !== "")
      ?.certification ?? "";
  ...
  return (
    <Box>
      {trailer && (
        <Modal
          sx={{ ".mantine-Modal-body": { padding: 0 } }}
          opened={opened}
          onClose={close}
          centered
            ...
            >
              <Box sx={{ display: "flex", flexDirection: "column" }}>
                <Flex gap="0.5rem">
                  ...
                  <Title
                    sx={{ color: "lightgrey", fontWeight: 400 }}
                    order={1}
                  >{`(${dayjs(releaseDate).format("YYYY")})`}</Title>
                </Flex>
                <Flex gap="xs">
                  {rating && (
                    <Badge
              ...
              {director && (
                <Flex direction="column">
                  <Text sx={{ lineHeight: 1.25 }} fz="md" fw={500}>
                    {director?.name}{" "}
                  </Text>
                  <Text sx={{ lineHeight: 1.25 }} fz="sm">
                    Director
                  </Text>
                </Flex>
              )}
